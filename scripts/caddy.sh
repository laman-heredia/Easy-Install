...
restore_caddyfile(){ if [[ -e "$BACKUP" ]]; then cp -a "$BACKUP" "$CADDYFILE"; else rm -f "$CADDYFILE"; fi; }
install_caddy(){ root_ubuntu; validate; confirm "安装 Caddy？" || die "已取消。"; export DEBIAN_FRONTEND=noninteractive; run apt-get update -q; run apt-get install -y caddy python3-minimal; [[ "$DRY_RUN" == true ]] && { ok "演练完成。"; return; }; write_caddyfile; systemctl enable --now caddy; if ! systemctl reload caddy; then restore_caddyfile; systemctl reload caddy || true; die "Caddy 新配置重载失败，已恢复原配置。"; fi; ok "Caddy 已启用：$DOMAIN"; }
