...
write_site(){ local name conf temp listen server_name location tls_flag; name="$(site_name)"; conf="/etc/nginx/sites-available/easy-install-${name}.conf"; [[ ! -e "$conf" || "$FORCE" == true ]] || die "站点已存在；使用 --force 更新。"; [[ -n "$ROOT_DIR" ]] || ROOT_DIR="$WEB_ROOT_BASE/$name"; listen="$PORT"; server_name="$DOMAIN"; [[ "$DOMAIN" == default ]] && { listen="$PORT default_server"; server_name="_"; }; if [[ -n "$PROXY_PASS" ]]; then location="proxy_pass ${PROXY_PASS};
...
EOF_CONF
chmod 644 "$temp"; mv "$temp" "$conf"; ln -sfn "$conf" "/etc/nginx/sites-enabled/easy-install-${name}.conf"; [[ "$name" != default ]] || rm -f /etc/nginx/sites-enabled/default; nginx -t; systemctl reload nginx; if [[ "$ENABLE_TLS" == true ]]; then [[ "$REDIRECT_HTTPS" == true ]] && tls_flag="--redirect" || tls_flag="--no-redirect"; certbot --nginx -d "$DOMAIN" --email "$EMAIL" --agree-tos --non-interactive "$tls_flag"; fi; ok "站点已启用：http://${DOMAIN}:$PORT"; }
install_nginx(){ root_ubuntu; validate; confirm "安装 Nginx？" || die "已取消。"; export DEBIAN_FRONTEND=noninteractive; run apt-get update -q; run apt-get install -y nginx; [[ "$ENABLE_TLS" == true ]] && run apt-get install -y certbot python3-certbot-nginx; [[ "$DRY_RUN" == true ]] && { ok "演练完成。"; return; }; install -d -m 755 "$MANAGED_DIR"; systemctl enable --now nginx; write_site; }
...
main(){ parse "$@"; validate; [[ "$VERBOSE" == true ]] && set -x; case "$COMMAND" in install) install_nginx;; add) add;; remove) remove;; list) list;; status) status;; uninstall) uninstall;; esac; }
main "$@"
