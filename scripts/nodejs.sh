...
service(){ root_ubuntu; validate; [[ -n "$APP_DIR" ]] || die "service 需要 --app-dir。"; [[ -d "$APP_DIR" ]] || die "应用目录不存在：$APP_DIR"; [[ -n "$USER_NAME" ]] || USER_NAME="www-data"; id "$USER_NAME" >/dev/null || die "用户不存在：$USER_NAME"; [[ -n "$SERVICE_NAME" ]] || SERVICE_NAME="easy-install-$(basename "$APP_DIR")"; local unit="/etc/systemd/system/${SERVICE_NAME}.service"; [[ ! -e "$unit" || "$FORCE" == true ]] || die "服务已存在；使用 --force 覆盖。"; if [[ "$DRY_RUN" == true ]]; then echo "[dry-run] write $unit and restart service"; return; fi; local temp; temp="$(mktemp /etc/systemd/system/.easy-install-node.XXXXXX)"; cat >"$temp" <<EOF_UNIT
[Unit]
