...
configure(){ [[ ! -e "$BACKUP" || "$FORCE" == true ]] || die "Redis 已由本脚本配置；使用 --force 更新。"; [[ -e "$BACKUP" ]] || cp -a "$CONF" "$BACKUP"; local temp; temp="$(mktemp /etc/redis/.redis.conf.XXXXXX)"; cp "$CONF" "$temp"; set_conf bind "$BIND" "$temp"; set_conf port "$PORT" "$temp"; set_conf protected-mode yes "$temp"; set_conf requirepass "$PASSWORD" "$temp"; set_conf maxmemory "$MAXMEMORY" "$temp"; set_conf maxmemory-policy "$POLICY" "$temp"; set_conf databases "$DATABASES" "$temp"; set_conf supervised systemd "$temp"; set_conf appendonly yes "$temp"; chown redis:redis "$temp"; chmod 640 "$temp"; redis-server "$temp" --test-memory 2 >/dev/null; mv "$temp" "$CONF"; if ! systemctl restart redis-server; then cp -a "$BACKUP" "$CONF"; systemctl restart redis-server || true; die "Redis 新配置启动失败，已恢复原配置。"; fi; }
...
uninstall(){ root_ubuntu; [[ "$DRY_RUN" != true ]] || die "uninstall 不支持 --dry-run。"; confirm "确定卸载 Redis？" || die "已取消。"; systemctl disable --now redis-server 2>/dev/null || true; rm -f "$BACKUP"; apt-get purge -y redis-server redis-tools; if [[ "$PURGE_DATA" == true ]]; then rm -rf /var/lib/redis /var/log/redis; else warn "Redis 数据仍保留在 /var/lib/redis。"; fi; ok "Redis 已卸载。"; }
