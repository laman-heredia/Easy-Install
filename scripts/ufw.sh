...
validate(){ valid_port "$SSH_PORT" || die "SSH 端口无效。"; [[ "$DEFAULT_INCOMING" =~ ^(deny|reject|allow)$ ]] || die "入站策略无效。"; [[ "$DEFAULT_OUTGOING" =~ ^(allow|deny|reject)$ ]] || die "出站策略无效。"; IFS=',' read -ra ports <<<"$ALLOW_PORTS"; for port in "${ports[@]}"; do [[ -z "$port" ]] && die "端口列表不能为空。"; valid_port "$port" || die "端口无效：$port"; done; }
